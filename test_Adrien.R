...
#regsubsets for gold
reg.exhaustive <- regsubsets(x=goldearned~.,data=teams.new.dt[,!c('win','deaths','goldspent','firstinhib','inhibkills','towerkills','firstbaron','firstdragon','matchid','teamid')]
                             ,really.big = T,method = 'exhaustive')
reg.backward <- regsubsets(x=firstinhib~.,data=teams.new.dt[,!c('matchid','teamid','win')],really.big = T,method = 'backward')
reg.forward <- regsubsets(x=firstinhib~.,data=teams.new.dt[,!c('matchid','teamid','win')],really.big = T,method = 'forward')
...
teams.scaled.dt <- as.data.table(sapply(teams.new.dt,scale,2))
teams.scaled.dt$win <- teams.new.dt$win #reassign win
...
#plotting
ClusteringPlot(teams.new.dt,'deaths','goldearned')
ClusteringPlot(teams.dt,'deaths','goldearned')
CompleteTeamModel_kCV(teams.new.dt,k=10,formula=' win~deaths+goldearned+I(deaths^2)+I(goldearned^2)')
CompleteTeamModel_kCV(teams.new.dt,k=10,formula=' win~deaths+goldearned')
CompleteTeamModel_kCV(teams.dt,k=10,formula=' win~deaths+goldearned')
CompleteTeamModel_kCV(teams.new.dt,k=10,formula=' win~deaths+kills+totminionskilled')
CompleteTeamModel_kCV(teams.scaled.dt,k=10,formula=' win~deaths+kills+totminionskilled')
