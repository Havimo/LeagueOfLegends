CreateChampPosition <- function(players.dt){
...
#Champ.Position.dt = subset(Champ.Position.dt, NbrGames > 100)
...
#              #labels = Champ.Position.dt$NamePOS)#,circle = TRUE)
#g <- g + scale_color_discrete(name = 'Position')
#print(g)

#PlotUsing ggbiplot 
#Nice plot
#g <- ggbiplot(pca, obs.scale = 1, var.scale =1, choices=c(1,3),
##              groups = Champ.Position.dt$position, ellipse = TRUE)
##             # labels = Champ.Position.dt$NamePOS)#,circle = TRUE)
##g <- g + scale_color_discrete(name = 'Position')
##print(g)
